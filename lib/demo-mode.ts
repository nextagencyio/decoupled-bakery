...
import specialsData from '@/data/mock/specials.json'
...
export function handleMockQuery(body: string): any {
  try {
    ...
    if (query.includes('GetSpecials') || query.includes('nodeSpecials')) {
      return loadMockData('specials.json')
    }
