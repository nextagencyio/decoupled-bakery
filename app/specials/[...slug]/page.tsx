...
import { GET_SPECIAL_BY_PATH } from '@/lib/queries'
import { DrupalSpecial } from '@/lib/types'
...
interface SpecialByPathData {
  route: {
    entity: DrupalSpecial
  } | null
}

async function getSpecial(path: string): Promise<DrupalSpecial | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<SpecialByPathData>({
      query: GET_SPECIAL_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/specials/${slug.join('/')}`
  const item = await getSpecial(path)

  if (!item) {
    return { title: 'Special Not Found | Bakery' }
  }

  return {
    title: `${item.title} | Bakery`,
    description: ((item as any).body?.processed ? (item as any).body.processed.replace(/<[^>]*>/g, '').substring(0, 160) : '') || `Learn more about ${item.title}.`,
  }
}

export default async function SpecialDetailPage({ params }: PageProps) {
  const { slug } = await params
  const path = `/specials/${slug.join('/')}`
  const item = await getSpecial(path)
