...
import { GET_MENU_ITEM_BY_PATH } from '@/lib/queries'
...
interface MenuItemByPathData {
  route: {
    entity: DrupalMenuItem
  } | null
}
...
async function getMenuItem(path: string): Promise<DrupalMenuItem | null> {
  try {
    ...
    const { data } = await apolloClient.query<MenuItemByPathData>({
      query: GET_MENU_ITEM_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/menu/${slug.join('/')}`
  const item = await getMenuItem(path)

  if (!item) {
    return { title: 'Menu Item Not Found | Bakery' }
  }
